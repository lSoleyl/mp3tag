...
module.run = function(argv, callback) {
  ...
  //Completion function
  var done = function(err,res) { if (typeof(callback) === 'function') callback(err,res) }
  ...
  if (_.some(tasks, (task) => { return task.type === 'help' })) {
    ...
    var catTasks = _.groupBy(tasks, (task) => { return task.type })
  ...
  var data; //The data to manipulate (gets set by the source task)
  //Callback to execute next task
  var nextTask = function nextTask(err) {
    //Error occurred, return it to the calling callback
    if (err)
      return done(err)

    if (tasks.length > 0) {
      var task = tasks.shift() //Take next task from list
      if (task.type !== 'source')
        return task.run(data, nextTask)

      return task.run(null, function(err, res) {
        if (err)
          return done(err)
...
module.defineTask('help', {
  ...
  var fieldSize = _.max(_.map(taskDefinitions, (task) => { return task.to_string().length } ))
  fieldSize += 5 //Some additional space between the argument and it's description

  //Group all commands by category
  var cat_commands = _.groupBy(taskDefinitions, (task) => { return task.type })
